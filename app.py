...
def webhook():
    ...
    try:
        ...
        if "message" in data and "text" in data["message"]:
            ...
            logger.info(f"📝 Обработка команды: '{text}' в чате {chat_id}")
            ...
            if text == "/start":
                ...
                logger.info("🔄 Обработка /all")
                send_all_links(chat_id)
            elif text == "/supports":
                logger.info("🔄 Обработка /supports")
                send_link('supports', chat_id)
            elif text == "/supplies":
                logger.info("🔄 Обработка /supplies")
                send_link('supplies', chat_id)
            elif text == "/database":
                logger.info("🔄 Обработка /database")
                send_link('database', chat_id)
            elif text == "/goods":
                logger.info("🔄 Обработка /goods")
                send_link('goods', chat_id)
            elif text == "/menu":
                logger.info("🔄 Обработка /menu")
                send_welcome(chat_id)
            else:
