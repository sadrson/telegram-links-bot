...
def send_telegram_message(text, chat_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "description" in error_info:
                logger.error(f"📝 Описание ошибки: {error_info['description']}")
...
def send_link(link_key, chat_id=None):
    ...
    if link_key in LINKS_DATABASE:
        ...
        logger.info(f"📝 Подготовлено сообщение для {link_key}")
...
def send_specific_link(link_key):
    """Отправляет конкретную ссылку"""
    logger.info(f"🌐 HTTP запрос на отправку: {link_key}")
...
def debug():
    ...
    if BOT_TOKEN and CHAT_ID:
        try:
            ...
            # Дополнительный тест с ссылкой
            test_success = send_link('supports')
            debug_info["supports_link_test"] = test_success
